 ...
 */
class JSValue(val v8Value: Any?, val context: JSContext) {
    ...
    fun valueForProperty(property: String): JSValue? {
        if (checkReleased()) { return null }
        if (this.isObject) {
            return JSValue((v8Value as? V8Object)?.get(property), this.context)
        }
        return null
    }
    ...
    fun valueAtIndex(index: Int): JSValue? {
        ...
        if (this.isArray) {
            ...
            return JSValue(v8Array.get(index), this.context)
        }
        return null
    }
    ...
    fun callWithArguments(arguments: List<Any>): JSValue? {
        ...
        return try {
            ...
            v8Arguments.release()
            return JSValue(result, this.context)
